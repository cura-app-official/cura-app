...
export default function SellingOrdersScreen() {
  ...
  useEffect(() => {
    ...
    const exists = TABS.some((tab) => tab.key === params.tab);
    if (exists) setActiveTab(params.tab);
  }, [params.tab]);
          ...
          {TABS.map((tab) => (
            <Pressable
              ...
              onPress={() => {
                ...
                router.setParams({ tab: tab.key });
              }}
              className={`px-5 py-3 items-center rounded-3xl ${
                activeTab === tab.key
                  ? "bg-accent"
